...
void TakePenultimateElement(Fraction* FractionArray, size_t Size)
{
	Fraction temp;
	std::ifstream readFile("Third.txt", std::ios::binary);
	if (!readFile)
	...
	int pos = (Size - 2) * sizeof(FractionArray[0]);
	readFile.seekg(pos, std::ios::beg);
...
int main()
{
	...
	// Take Penultimate Element
	TakePenultimateElement(temp2, Size);
}